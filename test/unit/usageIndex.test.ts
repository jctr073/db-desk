...
const col = (schema: string, table: string, column?: string): ColumnRef => ({ schema, table, column })
...
describe('buildUsageIndex', () => {
  it('indexes an annotation target under the "annotates" role', () => {
    const records: KnowledgeRecord[] = [
      { ...envelope, id: 'kn-a', kind: 'annotation', target: col('public', 'users', 'email'), text: 'PII' }
    ]
  ...
  it('normalizes keys to lowercase so differently-cased refs collide', () => {
    const records: KnowledgeRecord[] = [
      { ...envelope, id: 'kn-1', kind: 'annotation', target: col('Public', 'Users', 'Email'), text: 'a' },
      { ...envelope, id: 'kn-2', kind: 'annotation', target: col('public', 'users', 'email'), text: 'b' }
    ]
    const index = buildUsageIndex(records)
  ...
  it('indexes a table-level ref (no column) under the table key', () => {
    const records: KnowledgeRecord[] = [
      { ...envelope, id: 'kn-t', kind: 'annotation', target: col('public', 'users'), text: 'soft-deleted rows kept' }
    ]
    const index = buildUsageIndex(records)
  ...
  it('still indexes dangling refs (store does not resolve against live schema)', () => {
    const records: KnowledgeRecord[] = [
      { ...envelope, id: 'kn-d', kind: 'annotation', target: col('gone', 'dropped_table', 'ghost_col'), text: 'x' }
    ]
    const index = buildUsageIndex(records)
  ...
  it('ignores records of an unknown/forward-compat kind without throwing', () => {
    const records = [
      ...
      { ...envelope, id: 'kn-a', kind: 'annotation', target: col('public', 'users', 'email'), text: 'ok' }
  ...
  it('collects hits from multiple records touching the same column, preserving order', () => {
    const records: KnowledgeRecord[] = [
      { ...envelope, id: 'kn-a', kind: 'annotation', target: col('public', 'orders', 'user_id'), text: 'fk' },
      {
        ...envelope,
        id: 'kn-r',
        kind: 'relationship',
...
describe('lookupUsages', () => {
  const records: KnowledgeRecord[] = [
    ...
    { ...envelope, id: 'kn-col', kind: 'annotation', target: col('public', 'users', 'email'), text: 'PII' },
    // Table-level note referencing the whole users table
    { ...envelope, id: 'kn-tbl', kind: 'note', title: 't', body: 'b', references: [col('public', 'users')] }
  ]
  const index = buildUsageIndex(records)
