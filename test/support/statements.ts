...
export const READ_CASES: StatementCase[] = [
  ...
  {
    name: 'join',
    sql: 'SELECT c.email, o.reference FROM customers c JOIN orders o ON o.customer_id = c.id',
    expected: 'read',
    underReadOnly: 'permitted',
    mutates: false
  },
  ...
  {
    name: 'EXPLAIN ANALYZE of a select',
    sql: 'EXPLAIN (FORMAT TEXT, ANALYZE, BUFFERS) SELECT * FROM customers',
    expected: 'read',
    underReadOnly: 'permitted',
    mutates: false
  },
  {
    name: 'DDL keyword inside a string literal',
    sql: "SELECT 'DROP TABLE customers' AS spooky",
    expected: 'read',
    underReadOnly: 'permitted',
    mutates: false
  },
  {
    name: 'DML keyword inside a comment',
    sql: 'SELECT /* delete from customers */ 1',
    expected: 'read',
    underReadOnly: 'permitted',
    mutates: false
  },
  {
    name: 'DML keyword as a quoted identifier',
  ...
  {
    name: 'dollar-quoted DDL text',
    sql: 'SELECT $tag$ DROP TABLE customers $tag$',
    expected: 'read',
    underReadOnly: 'permitted',
    mutates: false
  },
  {
    name: 'DESCRIBE (Databricks)',
    sql: 'DESCRIBE customers',
    expected: 'read',
    underReadOnly: 'permitted',
    mutates: false,
    skipIntegration: 'DESCRIBE is not Postgres syntax'
  },
  {
    name: 'DESC (Databricks)',
    sql: 'DESC customers',
    expected: 'read',
    underReadOnly: 'permitted',
    mutates: false,
    skipIntegration: 'DESC is not Postgres syntax'
  },
  {
    name: 'backtick identifier (Databricks)',
    sql: 'SELECT `update` FROM customers',
    expected: 'read',
    underReadOnly: 'permitted',
    mutates: false,
    skipIntegration: 'backtick identifiers are not Postgres syntax'
  }
]
...
export const DML_CASES: StatementCase[] = [
  ...
  {
    name: 'INSERT',
    sql: "INSERT INTO customers (email, full_name) VALUES ('new@example.com', 'New')",
    expected: 'dml',
    underReadOnly: 'rejected',
    mutates: false
  },
  ...
  {
    name: 'top-level DML after a CTE',
    sql: 'WITH x AS (SELECT 1) DELETE FROM order_items',
    expected: 'dml',
    underReadOnly: 'rejected',
    mutates: false
  },
  ...
  },
  {
    ...
    sql: "EXPLAIN UPDATE orders SET status = 'paid'",
    expected: 'dml',
    underReadOnly: 'permitted',
    mutates: false,
    note: 'Postgres plans it happily -- no execution, no mutation. We block it anyway (decision C): EXPLAIN ANALYZE of the same statement would execute it, and we do not want the distinction to rest on parsing option lists.'
  },
  {
    name: 'EXPLAIN ANALYZE of an UPDATE',
    sql: "EXPLAIN (ANALYZE) UPDATE orders SET status = 'paid'",
    expected: 'dml',
    underReadOnly: 'rejected',
    mutates: false,
    note: 'ANALYZE really executes it; only the read-only session stops the write.'
  },
  {
    name: 'COPY FROM',
    sql: "COPY customers FROM '/tmp/customers.csv'",
    expected: 'dml',
    underReadOnly: 'rejected',
    mutates: false,
    skipIntegration: 'COPY needs a server-side file and the copy protocol'
  }
]
...
export const DDL_CASES: StatementCase[] = [
  {
    name: 'CREATE TABLE',
    sql: 'CREATE TABLE probe_created (a int)',
    expected: 'ddl',
    underReadOnly: 'rejected',
    mutates: false
  },
  ...
  {
    name: 'GRANT',
    sql: 'GRANT SELECT ON customers TO dbdesk_ro',
    expected: 'ddl',
    underReadOnly: 'rejected',
    mutates: false
  },
  {
    name: 'REVOKE',
    sql: 'REVOKE SELECT ON customers FROM dbdesk_ro',
    expected: 'ddl',
    underReadOnly: 'rejected',
    mutates: false
  },
  {
    name: 'COMMENT',
    sql: "COMMENT ON TABLE customers IS 'owned'",
    expected: 'ddl',
    underReadOnly: 'rejected',
    mutates: false
  },
  {
    ...
    sql: 'SELECT * INTO probe_into FROM customers',
    expected: 'ddl',
    underReadOnly: 'rejected',
    mutates: false
  },
  {
    name: 'VACUUM',
    sql: 'VACUUM customers',
    expected: 'ddl',
    underReadOnly: 'permitted',
    mutates: false,
    note: 'Not in the read-only session prohibition list -- it writes to disk but not to table data.'
  },
  {
    name: 'EXPLAIN of DDL',
    sql: 'EXPLAIN CREATE TABLE probe_created (a int)',
    expected: 'ddl',
    underReadOnly: 'rejected',
    mutates: false,
    skipIntegration: 'Postgres cannot EXPLAIN a CREATE TABLE'
  },
  {
    name: 'REFRESH MATERIALIZED VIEW',
    sql: 'REFRESH MATERIALIZED VIEW nothing',
    expected: 'ddl',
    underReadOnly: 'rejected',
    mutates: false,
    skipIntegration: 'no materialized view is seeded'
  }
]
...
export const UNKNOWN_CASES: StatementCase[] = [
  ...
  },
  {
    ...
    underReadOnly: 'rejected',
    mutates: false,
    note: 'LOCK defaults to ACCESS EXCLUSIVE, which a read-only transaction refuses (25006).'
  },
  {
    name: 'SELECT FOR UPDATE takes row locks',
    sql: 'SELECT * FROM customers FOR UPDATE',
    expected: 'unknown',
    underReadOnly: 'rejected',
    mutates: false
  },
  {
    name: 'DO block that writes',
    sql: "DO $$ BEGIN INSERT INTO customers (email, full_name) VALUES ('do@example.com', 'Do'); END $$",
    expected: 'unknown',
    underReadOnly: 'rejected',
    mutates: false
  },
  {
    name: 'SELECT calling a volatile function that writes',
    sql: "SELECT add_customer('volatile@example.com', 'Volatile')",
    expected: 'read',
    underReadOnly: 'rejected',
    mutates: false,
    note: 'THE case no client-side classifier can see: a plain SELECT that writes. Classified `read`, and correctly so -- only the server-side belt (or a read-only role) stops it. This is the documented residual risk.'
  },
  {
    name: 'two reads',
    ...
    underReadOnly: 'permitted',
    mutates: false,
    note: 'Harmless, but multi-statement input is refused wholesale -- see the escapes below.'
  },
  ...
  },
  {
    ...
    underReadOnly: 'rejected',
    mutates: false,
    skipIntegration: 'no procedure is seeded'
  },
  {
    name: 'USE catalog (Databricks)',
    sql: 'USE CATALOG main',
    expected: 'unknown',
    underReadOnly: 'rejected',
    mutates: false,
    skipIntegration: 'USE CATALOG is not Postgres syntax'
  }
]
...
export const PG_CASES: StatementCase[] = ALL_CASES.filter((c) => !c.skipIntegration)
