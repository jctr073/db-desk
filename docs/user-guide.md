...
DB Desk provides one hierarchical **Manage Catalogs and Schemas…** dialog from
either a Databricks connection or catalog's right-click menu. Uncheck a catalog
to hide it and everything below it, or leave the catalog enabled and expand it
to choose individual schemas. Unselected objects are not introspected: they
stay out of the schema tree, the AI Agent's schema summary, and the agent's
schema search and describe tools. Raw SQL can still query them.

A catalog with no saved schema selection loads everything, except that a large
catalog (more than 25 schemas) opens the hierarchy instead of loading; a
pinned catalog shows a faint "12 of 240" count on its row. Checking every item
clears the selection, so schemas created later appear again automatically. For
a large catalog, DB Desk keeps the current full list explicit so saving does not
immediately reopen the hierarchy.
...
- choose which catalogs and schemas are shown (Databricks,
